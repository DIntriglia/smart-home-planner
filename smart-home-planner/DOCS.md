...
- **HA availability** aggregates relevant enabled entities into Available, Partially unavailable, Unavailable, or Unknown. Entity values such as `off` and `0` count as available. Disabled entities and action-only button, input-button, and scene entities are excluded. Event entities (including Matter buttons) are included: a last-event timestamp counts as available regardless of its age, while explicit unavailable, unknown, or missing states remain unavailable or unknown. Freshness uses the HA snapshot time, not the last button press. Diagnostic sensors are included; one failing sensor can produce Partially unavailable. Open the device's availability details for affected names and entity IDs.
...
The dashboard's **Home Assistant diagnostics** shows Working devices that have unavailable entities past the grace period, Working devices disabled in HA, and Working devices whose availability is Unknown. Each list links to the matching Devices filters for review and bulk editing. Category cards start collapsed with a count and chevron. Expand a card to see all matching devices; expansion is preserved during background refresh. Each card includes a Review / bulk edit link. Set the grace period under **Settings → Home Assistant Integration → Availability diagnostics** (default 120 seconds, allowed 0–3600 seconds). Raw availability remains visible during the grace period; it only delays the Needs attention list. At least one unavailable entity must have remained unavailable for the grace period. Background refresh preserves selections still matching your filters and drops selections that no longer match.
...
Device forms show source information at the top, before Basic Information. HA-disabled devices have an amber warning matching the existing form warnings. Availability details remain collapsed until expanded; manually entered devices do not show HA-only details.
...
Ring chime sound controls that explicitly report Unknown without supporting on/off feedback produce a warning on the device, identifying the entity. They do not affect the availability totals or dashboard audit. They can still be triggered in Home Assistant. Missing states, explicit Unavailable states, other Ring sirens, and unknown sensors remain diagnostic issues. Device availability details identify both unavailable and unknown entities. If only stateless controls exist, the planner cannot infer device health and says so.

When a fresh entity registry confirms every entity of a device is disabled, availability is **Not monitored — all entities disabled in Home Assistant**. This is normal for hubs with optional diagnostic entities and does not mean the hub itself is disabled or offline. These devices are omitted from the availability audit. Linked records require this confirmation for every linked HA device. Re-enabling an entity resumes monitoring; missing or stale metadata remains Unknown.

Dashboard audit rows include integration domains, separate available/unavailable/unknown counts, and names and IDs of affected entities (up to three per category; open the device for the full list). All monitored entities unavailable is distinct from a partial entity issue and does not prove the physical device has failed. All-entity outage duration starts when the last monitored entity became unavailable; partial issues show the longest current entity outage. Durations use HA state-change timestamps and are omitted when timestamps are missing or invalid. Expanded category cards show every matching device and provide a styled Review / bulk edit link preserving the review filters.

Audit summaries omit zero counts to reduce clutter; device availability details retain the full breakdown. Each diagnostic category has its own collapsible card.

Device add/edit pages keep their action bar at the bottom of the screen. Apply saves without leaving; Save returns to Devices. Unsaved edits trigger a discard confirmation for in-app links and the browser confirmation when unloading the page (browser support applies, including embedded Home Assistant navigation). Saving locks the form briefly to prevent edits being lost during a pending write. Failed saves retain unsaved-change protection.
